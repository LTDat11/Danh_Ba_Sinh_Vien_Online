    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.apply {
            ...
            btnDate.setOnClickListener {
                ...
                val datePickerDialog = DatePickerDialog(this@AddActivity, DatePickerDialog.OnDateSetListener { _, yearSelected, monthOfYear, dayOfMonth ->
                    ...
                    if (ageDifference < 18) {
                        ...
                        Toast.makeText(this@AddActivity, "Bạn chưa đủ 18 tuổi", Toast.LENGTH_SHORT).show()
                    } else {
            ...
            btnSaveInfo.setOnClickListener {
                ...
                if (!isNetworkConnected()){
                    Toast.makeText(this@AddActivity,"Vui lòng kiểm tra kết nối mạng và thử lại", Toast.LENGTH_SHORT).show()
                }else{
                    ...
                    if (name.isEmpty() || phoneNumber.isEmpty() || email.isEmpty() || studentId.isEmpty()) {
                        ...
                        Toast.makeText(this@AddActivity, "Vui lòng nhập đầy đủ thông tin bắt buộc(Tên, số đt, email, mã số)", Toast.LENGTH_SHORT).show()
                            ...
                            CoroutineScope(Dispatchers.IO).launch {
                                withContext(Dispatchers.Main) {
                                    if (!isValidPhoneNumber(phoneNumber)) {
                                        ...
                                        Toast.makeText(this@AddActivity, "Số điện thoại không hợp lệ(ít nhất 10 số)", Toast.LENGTH_SHORT).show()
                                        return@withContext
                                    }

                                    if (!isValidEmail(email)) {
                                        binding.progressBar.visibility = View.GONE
                                        Toast.makeText(this@AddActivity, "Email không hợp lệ", Toast.LENGTH_SHORT).show()
                                        return@withContext
                                    }

                                    if (!isDateOfBirthValid(dateOfBirth)) {
                                        binding.progressBar.visibility = View.GONE
                                        Toast.makeText(this@AddActivity, "Ngày sinh không hợp lệ, chưa đủ 18 tuổi", Toast.LENGTH_SHORT).show()
                                        return@withContext
                                    }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            withContext(Dispatchers.Main) {
                ...
                if (isStudentInfoExist(studentId,email,phoneNumber)){
                    Toast.makeText(this@AddActivity, "Thông tin sinh viên đã tồn tại. Vui lòng kiểm tra lại.", Toast.LENGTH_SHORT).show()
                    binding.progressBar.visibility = View.GONE
                }else{
                    if (isStudentIdExist(studentId)){
                        Toast.makeText(this@AddActivity, "Mã số sinh viên đã tồn tại. Vui lòng kiểm tra lại.", Toast.LENGTH_SHORT).show()
                        binding.progressBar.visibility = View.GONE
                    }else if (isStudentEmailExist(email)){
                        Toast.makeText(this@AddActivity, "Email đã tồn tại. Vui lòng kiểm tra lại.", Toast.LENGTH_SHORT).show()
                        binding.progressBar.visibility = View.GONE
                    }else if (isStudentPhoneNumberlExist(phoneNumber)){
                        Toast.makeText(this@AddActivity, "Số điện thoại đã tồn tại. Vui lòng kiểm tra lại.", Toast.LENGTH_SHORT).show()
                        binding.progressBar.visibility = View.GONE
                    }else{
                            ...
                            userUid?.let{
                                ...
                                studentCollectionRef.add(studentInfo)
                                    .addOnSuccessListener {
                                        Toast.makeText(this@AddActivity, "Lưu thành công", Toast.LENGTH_SHORT).show()
                                        binding.progressBar.visibility = View.GONE
                                        resetForm()
                                    }.addOnFailureListener { e ->
                                        Toast.makeText(this@AddActivity, "Lỗi: $e", Toast.LENGTH_SHORT).show()
                                        binding.progressBar.visibility = View.GONE
                                    }
